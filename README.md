...
- Sixel 图片随终端内容一起滚动。
- **SFTP**：浏览远程目录，支持上传、下载和服务器之间的 P2P 文件传输，可查看进度并取消传输任务。
- **Zmodem**：内置 `rz` / `sz` 文件传输，支持多层 SSH 跳板场景。

![Nutty：Sixel 终端图片预览](sixel.png)

![Nutty：SFTP 远程文件浏览器](sftp.png)

![Nutty：Zmodem 文件选择框与完整终端窗口](zmodem.png)

## SSH 隧道

通过 Tunnel 面板配置 SSH 本地转发与远程转发，设置监听地址和端口，并管理当前会话的转发规则，便于访问远程内网服务、数据库和开发服务。

![Nutty：SSH 隧道管理，使用本地示例地址](tunnel.png)
