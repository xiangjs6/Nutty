...
| 操作 | macOS | 说明 |
| --- | --- | --- |
| 切换到第 1-10 个标签页 | `Cmd+1` ... `Cmd+0` | `0` 表示第 10 个标签页 |
| 粘贴 | `Cmd+v` | 搜索框获取焦点时不会处理该操作 |
| 复制选中内容 | `Cmd+c` | 复制当前终端选中的文本 |
| 按页滚动终端历史 | `Shift+PageUp` / `Shift+PageDown` | 仅本地滚动，不会发送到 shell |
| 调整字体大小 | `Cmd+=` / `Cmd+-` | 字体范围为 `6-46` |
| 左右分屏 | `Cmd+s` | 在当前标签页中创建一个新的终端面板 |
| 上下分屏 | `Cmd+'` | 如果当前是 SSH 会话，新面板会复用该连接 |
| 拖拽终端面板 | `Cmd+鼠标左键拖拽` | 至少需要两个面板；拖到其他面板可交换，拖到标签栏可新建标签页 |
| 打开主机搜索 | `Cmd+t` | 打开主机选择器（SSH / 本地 / 历史记录） |
| 打开搜索 | `Cmd+f` | 聚焦到搜索输入框 |
| 查找下一个 / 上一个 | `Cmd+]` / `Cmd+[` | 基于当前搜索内容 |
| 退出搜索 | `Enter` / `Esc` | `Enter` 保留搜索，`Esc` 退出搜索模式 |
